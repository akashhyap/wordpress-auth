 ...
 * @return {number}
 */
export const getFloatVal = (string: string): number => {
    const floatValue = string.match(/[+-]?\d+(\.\d+)?/g)?.[0] ?? '';
    return floatValue ? parseFloat(parseFloat(floatValue).toFixed(2)) : 0;
};
 ...
 */
export const getFormattedCart = (data: any): FormattedCart | null => {
    let formattedCart: FormattedCart | null = null;
